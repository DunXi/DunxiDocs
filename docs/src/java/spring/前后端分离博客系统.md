...
<!-- ![image-20220227224537701](E:/JAVA学习/SGBlog/笔记/img/image-20220227224537701.png) -->

<!-- ![image-20220227224443813](E:/JAVA学习/SGBlog/笔记/img/image-20220227224443813.png) -->



秘钥

<!-- ![image-20220228230512598](E:/JAVA学习/SGBlog/笔记/img/image-20220228230512598.png) -->

<!-- ![image-20220228230933808](E:/JAVA学习/SGBlog/笔记/img/image-20220228230933808.png) -->
...
<!-- ![image-20220313133714102](E:/JAVA学习/SGBlog/笔记/img/image-20220313133714102.png) -->
    ...
    public void run(String... args) throws Exception {
        ...
        Map<String, Integer> viewCountMap = articles.stream()
                .collect(Collectors.toMap(article -> article.getId().toString(), article -> {
                    return article.getViewCount().intValue();//Long 转换成Integer
                }));
        //存储到redis中
        redisCache.setCacheMap(SystemConstants.ARTICLE_VIEW_KEY,viewCountMap);
    }
}
```
...
​	在启动类上面加上注解@EnableScheduling//Spring自带的定时任务

```java
@SpringBootApplication//启动类
@EnableScheduling//Spring自带的定时任务
@MapperScan("com.dunxi.mapper")//扫描mapper
public class DunXiBlogApplication {
    public static void main(String[] args) {
        SpringApplication.run(DunXiBlogApplication.class,args);
    }
...
在framework中新建job.UpdateViewCountJob定时任务

​	并在方法上加上 @Scheduled(cron = "0/5 * * * * ?")注解  里面的cron表达式表示每隔55秒执行一次
...
​	SQL脚本：DXBlog\资源\SQL\dx_tag.sql
...
```java
package com.dunxi.domain.entity;
...
```java
package com.dunxi.mapper;

import com.baomidou.mybatisplus.core.mapper.BaseMapper;
import com.dunxi.domain.entity.Tag;
...
```java
package com.dunxi.service.impl;

import com.baomidou.mybatisplus.extension.service.impl.ServiceImpl;
import com.dunxi.domain.entity.Tag;
import com.dunxi.mapper.TagMapper;
import com.dunxi.service.TagService;
import org.springframework.stereotype.Service;
...
```java
package com.dunxi.config;

import com.dunxi.filter.JwtAuthenticationTokenFilter;
import org.springframework.beans.factory.annotation.Autowired;
...
//                .antMatchers("/logout","/comment","/user/userInfo").authenticated()
                //jwt过滤器测试用，如果测试没有问题吧这里删除了
//                .antMatchers("/link/getAllLink","/comment").authenticated()//需要认证之后才能去访问
                // 除上面外的所有请求全部不需要认证即可访问
                .anyRequest().permitAll();
...
```java
package com.dunxi.filter;

import com.alibaba.fastjson.JSON;
import com.dunxi.domain.ResponseResult;
import com.dunxi.domain.entity.LoginUser;
import com.dunxi.enums.AppHttpCodeEnum;
import com.dunxi.utils.JwtUtil;
import com.dunxi.utils.RedisCache;
import com.dunxi.utils.WebUtils;
...
​	建表SQL及初始化数据见：DXBlog\资源\SQL\dx_menu.sql
...
public class TagController {
    ...
    @PostMapping
    public ResponseResult addTag(@RequestBody TagListDto dto){
...
public interface TagService extends IService<Tag> {
    ...
    ResponseResult addTag(TagListDto dto);
}
```
...
```java
@Override
public ResponseResult addTag(TagListDto dto) {
...
```java
@PutMapping
public ResponseResult changeTag(@RequestBody TagDto dto){
    return tagService.changeTag(dto);
}
```
...
```java
ResponseResult changeTag(TagDto dto);
```
...
```java
@Override
public ResponseResult changeTag(TagDto dto) {
    Tag tag = BeanCopyUtils.copyBean(dto,Tag.class);
...
​	SQL脚本：DXBlog\资源\SQL\dx_article_tag.sql
...
Article 设置创建时间创建人修改时间修改人可以自动填充
...
```java
@Override
@Transactional//事务注解 执行过程中出现了异常 操作回滚
...
public void export(HttpServletResponse response){
    ...
    try {
        ...
        List<ExcelCategoryVo> categoryVos = BeanCopyUtils.copyBeanList(list, ExcelCategoryVo.class);
        //把数据写入到Excel中
...
在方法上加上@PreAuthorize("@ps.hasPermission('content:category:export')")注解表示这个是分类导出的权限
...
public ResponseResult getArticleInfo(Long id) {
    ...
    List<ArticleTag> articleTags = articleTagService.getByArticleId(id);
    List<String> list = new ArrayList<>();
    for (ArticleTag articleTag : articleTags) {
        list.add(articleTag.getTagId().toString());
    }
...
```java
@PutMapping
public ResponseResult change(@RequestBody Menu menu){
...
```java
ResponseResult change(Menu menu);
```
...
```java
@Override
public ResponseResult change(Menu menu) {
    if (menu.getId().equals(menu.getParentId())){
        throw new SystemException(AppHttpCodeEnum.CORRECT_ID_NOT_PARENT);
    }
    updateById(menu);
    return ResponseResult.okResult();
}
...
public ResponseResult delete(Long menuId) {
    ...
    if (count > 0){
        throw new SystemException(AppHttpCodeEnum.CHILD_NOT_NULL);
    }
...
```java
ResponseResult getList(Integer pageNum, Integer pageSize, String roleName, String status);
```
...
public ResponseResult changeStatus(@RequestBody RoleStatusDto dto){
...
```java
ResponseResult changeStatus(RoleStatusDto dto);
```
...
```java
@Override
public ResponseResult changeStatus(RoleStatusDto dto) {
    updateById(new Role(dto.getRoleId(),dto.getStatus()));
    return ResponseResult.okResult();
}
```
...
public class MenuTreeVo {
    ...
    public void setMenuName(String label) {
        this.label = label;
    }
}
```

​	因为前段需要的label是对应数据库里的menuName字段，我们定义一个label用来接收menuName，定义一个set get方法把menuName的值放到label上面。
...
public ResponseResult getTreeSelect() {
    List<Menu> menus = baseMapper.selecAllRouterMenu();
    List<MenuTreeVo> menuTreeVos = BeanCopyUtils.copyBeanList(menus, MenuTreeVo.class);

    List<MenuTreeVo> menuTreeVos1 = builderMenuTreeSelect(menuTreeVos, 0l);

    return ResponseResult.okResult(menuTreeVos1);
}

private List<MenuTreeVo> builderMenuTreeSelect(List<MenuTreeVo> menuTreeVos, long parentId) {
    List<MenuTreeVo> menuTree = menuTreeVos.stream()
            .filter(menu -> menu.getParentId().equals(parentId))
            .map(menu -> {menu.setChildren(getChildrenSelect(menu, menuTreeVos)); return menu;})
            .collect(Collectors.toList());
    return menuTree;
}

private List<MenuTreeVo> getChildrenSelect(MenuTreeVo menu, List<MenuTreeVo> menus) {
    List<MenuTreeVo> childrenList = menus.stream()
            .filter(m -> m.getParentId().equals(menu.getId()))
            .map(m -> { m.setChildren(getChildrenSelect(m,menus)); return m ;})
            .collect(Collectors.toList());
    return childrenList;
}
```

先从数据库里面查出Menu对象的集合，然后使用BeanCopyUtils工具类转换成VO对象的集合，然后利用stream流构建树。
...
public class RoleServiceImpl extends ServiceImpl<RoleMapper, Role> implements RoleService {
    ...
    public ResponseResult addRole(AddRoleDto addRoleDto) {
        ...
        //保存成功之后自动把数据库里自增的id放到实体类里面
        save(role);

        //在下面需要拿ID的时候直接用role.getId就可以拿到新增的id了
...
public ResponseResult getRoleMenuTree(Long id) {
    ...
    List<MenuTreeVo> menuTreeVos = BeanCopyUtils.copyBeanList(menus, MenuTreeVo.class);
    //构建树
    List<MenuTreeVo> menuTreeVoList = builderMenuTreeSelect(menuTreeVos, 0l);
    ...
    return ResponseResult.okResult(new RoleMenuTreeVo(menuTreeVoList,checkedKeys));
}
...
```java
ResponseResult update(UpdateRoleDto updateRoleDto);
```
...
```java
@Override
public ResponseResult update(UpdateRoleDto updateRoleDto) {
...
```java
@PostMapping
public ResponseResult addUser(@RequestBody AddUserDto addUser){
...
```java
ResponseResult addUser(AddUserDto addUser);
```
...
@Override
public ResponseResult addUser(AddUserDto addUser) {
    ...
    List<UserRole> userRoleList = addUser.getRoleIds().stream()
...
private void filterUserInfo(User user){
    //对数据进行非空判断
    if (!StringUtils.hasText(user.getUserName())){
        throw new SystemException(AppHttpCodeEnum.USERNAME_NOT_NULL);
    }
...
@Override
public ResponseResult delete(Long id) {
    //先判断删除的是不是当前登录的用户，如果是 报错
    Long userId = SecurityUtils.getUserId();
    if (id.equals(userId)){
        throw new SystemException(AppHttpCodeEnum.CURRECT_LOGINUSER);
    }
    //删除用户
    baseMapper.deleteById(id);
...
public ResponseResult getUserInfo(Long id) {
    ...
    ResponseResult<List<RoleAllListVo>> result = roleService.listAllRole();

    List<RoleAllListVo> roles = result.getData();

    User user = baseMapper.selectById(id);
    UserInfoVo userInfo = BeanCopyUtils.copyBean(user, UserInfoVo.class);

    return ResponseResult.okResult(new UpdateUserVo(roleIds,roles,userInfo));
}
```
...
```java
ResponseResult update(UpdateUserDto updateUserDto);
```
...
```java
@Override
public ResponseResult update(UpdateUserDto updateUserDto) {
    User user = BeanCopyUtils.copyBean(updateUserDto, User.class);
    updateById(user);

    //删除用户与角色的关联信息
    LambdaQueryWrapper<UserRole> wrapper = new LambdaQueryWrapper<>();
    wrapper.eq(UserRole::getUserId,updateUserDto.getId());
    userRoleService.remove(wrapper);


    List<UserRole> userRoleList = updateUserDto.getRoleIds().stream()
            .map(rid -> new UserRole(updateUserDto.getId(), rid))
            .collect(Collectors.toList());
    userRoleService.saveBatch(userRoleList);
...
public ResponseResult getPageList(Integer pageNum, Integer pageSize, String name, String status) {
    ...
    Page page = page(new Page(pageNum, pageSize), wrapper);
...
```java
@PutMapping
public ResponseResult update(@RequestBody UpdateCategoryDto categoryDto){
...
```java
ResponseResult update(UpdateCategoryDto categoryDto);
```
...
```java
@Override
public ResponseResult update(UpdateCategoryDto categoryDto) {
...
public class LinkController {
    ...
    @GetMapping("/list")
    public ResponseResult list(Integer pageNum, Integer pageSize, String name, String status){
        return linkService.findAll(pageNum,pageSize,name,status);
    }
}
```
...
```java
ResponseResult findAll(Integer pageNum, Integer pageSize, String name, String status);
```
...
```java
@Override
public ResponseResult findAll(Integer pageNum, Integer pageSize, String name, String status) {
    ...
    Page<Link> page = page(new Page(pageNum,pageSize),wrapper);
...
public ResponseResult update(@RequestBody UpdateLinkDto updateLinkDto){
    return linkService.update(updateLinkDto);
}
```
...
```java
ResponseResult update(UpdateLinkDto updateLinkDto);
```
...
```java
@Override
public ResponseResult update(UpdateLinkDto updateLinkDto) {
